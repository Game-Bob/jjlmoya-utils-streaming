...
import { streamingCategory } from "../../../../category";
...
export function getStaticPaths() {
    return LANGUAGE_CODES.map((locale) => ({
        params: { locale, vertical: "streaming" },
        props: { locale },
    }));
}
...
const buildUrlEntry = async (locale: Language, path: string, index: number): Promise<string> => {
    const links = await Promise.all(LANGUAGE_CODES.map(async (alternateLocale) => {
        const alternateCategory = await getContent(streamingCategory, alternateLocale);
...
export const GET: APIRoute = async ({ props }) => {
    ...
    const category = await getContent(streamingCategory, locale);
